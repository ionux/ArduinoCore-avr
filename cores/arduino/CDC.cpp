...
size_t Serial_::write(const uint8_t *buffer, size_t size)
{
	...
	// is open (not just the pipe) - the OS should set lineState when
    // the port is opened and clear lineState when the port is closed.
    // Bytes sent before the user opens the connection or after the
    // connection is closed are lost - just like with a UART.

	// TODO - ZE - check behavior on different OSes and test what happens
    // if an open connection isn't broken cleanly (cable is yanked out,
    // host dies or locks up, or host virtual serial port hangs).
